import Profile, { updateOne, findOne } from '../models/profile';
...
export function upDataProfile(req, res, next) {
    ...
    updateOne({ _id: req.body.id, creator: req.userData.userId }, newProfile)
        .then(result => {
...
export function getProfile(req, res, next) {
    ...
    findOne({ creator: id }, function (err, pro) {
        if (err)
